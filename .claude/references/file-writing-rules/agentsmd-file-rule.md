...
- 仕様変更やリリース・メジャーアップデート時には必ず変更履歴（docs/change-log/CHANGELOG.md）を更新する
...
- テストの決済情報はカード番号は`4242 4242 4242 4242`、名前は`test`にする。他の情報は適当でOK。
- その他のテスト中の情報が必要な場合は、エージェントが資格情報を推測・入力せず、必ずユーザーに尋ねる(認証情報の入力やログイン操作はユーザーの指示・実施を仰ぐ)。
...
- ドメイン上の重要ルールの分冊（`docs/domain-rules/*.md`。AGENTS.md の索引行は不変則が変わったときだけ）
- カスタマーサポート用ナレッジベース（`custumer-support/`）
- 変更履歴（`docs/change-log/CHANGELOG.md`）
